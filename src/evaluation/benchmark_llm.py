...
TEST_CASES = [
    ...
    {"category": "STRUCTURED", "question": "List all contractors in the company"},
    ...
    {"category": "GRAPH", "question": "Who is the direct supervisor of Alexandra Chen?"},
    # GRAPH French
...
def query_model(model: str, prompt: str) -> dict:
    ...
    try:
        ...
        return {"answer": "", "latency": round(time.time() - t0, 2), "error": str(e)}

def run_benchmark():
    print("=" * 70)
    print("MULTI-LLM BENCHMARK — Qwen2.5:7b vs DeepSeek-R1:7b")
    print(f"Total questions: {len(TEST_CASES)} x {len(MODELS)} models = {len(TEST_CASES)*len(MODELS)} calls")
    print("=" * 70)
    ...
    for cat in categories:
        ...
        for model in MODELS:
            ...
            if latencies:
                ...
                print(f"  {model:<20} avg: {avg}s | min: {mn}s | max: {mx}s")

    os.makedirs("benchmarks", exist_ok=True)
    with open("benchmarks/llm_comparison.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"\nResults saved to benchmarks/llm_comparison.json")
    print("=" * 70)
